...
def mousepos(event):
    ...
    if mx<800:
        ...
        g.bind("<Button-1>", second)
def remove(event):
    m2x=int(math.ceil(event.x / 100.0)) * 100 -100           #finds the top left corner of a square
    m2y=int(math.ceil(event.y / 100.0)) * 100 -100
    if m2x==mx and m2y==my:                                  #compares the two squares clicked, if it is the same
        g.delete("mSelected", "txt")                         #Delete the green square selecting the piece
def second(event):
    m2x=int(math.ceil(event.x / 100.0)) * 100 -100           #finds the top left corner of a square
    m2y=int(math.ceil(event.y / 100.0)) * 100 -100
